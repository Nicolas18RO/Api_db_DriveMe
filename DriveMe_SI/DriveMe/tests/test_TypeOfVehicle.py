...
class TypeOfVehicleTestCase(APITestCase):
    ...
    def test_list_type_of_vehicle(self):
        response = self.client.get("/api/type_of_vehicle/")
    ...
    def test_create_type_of_vehicle(self):
        ...
        response = self.client.post("/api/type_of_vehicle/", data, format='json')
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        print("Create response data:", response.data)

    def test_update_type_of_vehicle(self):
        update_data = {"name_type_vehicle": "Camioneta 4x4"}
        url = f"/api/type_of_vehicle/{self.type_of_vehicle.id_type_vehicle}/"
